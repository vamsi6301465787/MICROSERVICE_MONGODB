...
const ApiStudentKeys: string[] = ['Id', 'rollnumber', 'email', 'password', 'isExist']

function modelApiStudentFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.Student {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiStudentKeys.forEach(k => knownKeys[k] = true)

	const result: Api.Student = {
		'Id': stringFromJson(`${name}.Id`, value['Id']),
		'rollnumber': stringFromJson(`${name}.rollnumber`, value['rollnumber']),
...
function modelApiStudentToJsonContent(name: string, value: Api.Student, knownKeys: Record<string, boolean> = {}): ToJson<Api.Student> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.Student> = {
		'Id': stringToJson(`${name}.Id`, value['Id']),
		'rollnumber': stringToJson(`${name}.rollnumber`, value['rollnumber']),
...
export function modelApiStudentFromJson(name: string, value: any): Api.Student {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.Student = modelApiStudentFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			// throw `Unexpected key: ${key}`
			console.warn(`Unexpected key in Api.Student: ${key}`)
		}
	}
...
export function modelApiStudentToJson(name: string, value: Api.Student): ToJson<Api.Student> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.Student> = modelApiStudentToJsonContent(name, value, knownKeys)
...
const ApiStudentUpdateKeys: string[] = ['rollnumber', 'email', 'password']

function modelApiStudentUpdateFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.StudentUpdate {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}

	ApiStudentUpdateKeys.forEach(k => knownKeys[k] = true)

	const result: Api.StudentUpdate = {
		'rollnumber': allowUndefined(stringFromJson)(`${name}.rollnumber`, value['rollnumber']),
...
function modelApiStudentUpdateToJsonContent(name: string, value: Api.StudentUpdate, knownKeys: Record<string, boolean> = {}): ToJson<Api.StudentUpdate> {
	if (typeof value !== 'object' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected object got ${typeof value}`
	}
	...
	const result: ToJson<Api.StudentUpdate> = {
		'rollnumber': allowUndefined(stringToJson)(`${name}.rollnumber`, value['rollnumber']),
...
export function modelApiStudentUpdateFromJson(name: string, value: any): Api.StudentUpdate {
	const knownKeys: Record<string, boolean> = {}
	const result: Api.StudentUpdate = modelApiStudentUpdateFromJsonContent(name, value, knownKeys)
	...
	for (const key of Object.keys(value)) {
		if (!knownKeys[key]) {
			// throw `Unexpected key: ${key}`
			console.warn(`Unexpected key in Api.StudentUpdate: ${key}`)
		}
	}
...
export function modelApiStudentUpdateToJson(name: string, value: Api.StudentUpdate): ToJson<Api.StudentUpdate> {
	const knownKeys: Record<string, boolean> = {}
	const result: ToJson<Api.StudentUpdate> = modelApiStudentUpdateToJsonContent(name, value, knownKeys)
